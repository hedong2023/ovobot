...
@REM decide to genarate whole project or a single board
for /F "tokens=1-2 delims=\." %%a in ("%project%") do (
    set target=%%a
    if %%b neq "" set board=%%b
)

if defined board (
    echo:
    echo Generate %target% %board% fabrication outputs
    if exist ..\fabs\%target% (
        rmdir /s /q ..\fabs\%target%
    )
    call :gen %target% %board%
) else (
    if exist ..\fabs\%target% (
        rmdir /s /q ..\fabs\%target%
    )
    for /f %%i in ('dir /ad /b ..\%target%') do (
        echo:
        echo Generate %target% %%i fabrication outputs
        call :gen %target% %%i
    )
)
endlocal
