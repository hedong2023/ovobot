...
if %cmd%==gen (
    for /F "tokens=1,2,3 delims=###" %%i in ('python get-tag-detail.py %args%') do (
        echo Generate %%i %%j fabrication outputs
        if exist ..\fabs\%%i (
            rmdir /s /q ..\fabs\%%i
        )
        call :gen %%i cleanrobot-square-%%j %%k
    )
)
...
for /F "tokens=1 delims=V,v" %%a in ("%ver%") do (
    if not %3 == %%a (
        echo Tag version not match the pcb!
        exit 1
    )
)
